...
def load_sensor_data(apps, schema_editor):
    Variables = apps.get_model('geowebapp', 'Variables')
    CV_VariableType = apps.get_model('geowebapp', 'CV_VariableType')
    CV_VariableName = apps.get_model('geowebapp', 'CV_VariableName')
    CV_Speciation = apps.get_model('geowebapp', 'CV_Speciation')

    CV_UnitsType = apps.get_model('geowebapp', 'CV_UnitsType')
    Units = apps.get_model('geowebapp', 'Units')
    ...
    CV_Status = apps.get_model('geowebapp', 'CV_Status')

    Results = apps.get_model('geowebapp', 'Results')
    CV_Medium = apps.get_model('geowebapp', 'CV_Medium')
    ...
    vwc_variable_name = CV_VariableName.objects.filter(
        term='volumetricWaterContent').first()
    ...
    variable_unit_type = CV_UnitsType.objects.filter(
        term='concentrationVolumePerVolume').first()
    vwc_units = Units(
    ...
    temperature_variable_name = CV_VariableName.objects.filter(
        term='temperature').first()
    ...
    temperature_variable_unit_type = CV_UnitsType.objects.filter(
        term='concentrationVolumePerVolume').first()
    ...
    for file_path in sensors_data_files:
        ...
        sampling_feature = SamplingFeatures.objects.filter(
            samplingfeaturecode=f"{sensor_location}").first()
        ...
        for data in sensor_data.itertuples():
            result_time = [int(date_component)
                           for date_component in data.Date.split('/')]

            soil_moisture_results = Results(
                resultuuid=str(uuid.uuid4()),
                resultdatetime=timezone.make_aware(datetime(
                    result_time[2], result_time[0], result_time[1], 0, 0, 0)),
            ...
            soil_moisture_timeseries_results_values = TimeSeriesResultValues(
                ...
                valuedatetime=timezone.make_aware(datetime(
                    result_time[2], result_time[0], result_time[1], 0, 0, 0)),
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("geowebapp", "0007_auto_20250612_1530"),
    ]
